...
- [x] 1.1 Add `mark_incomplete_structures(candidates)` helper to
...
- [x] 1.2 In `app/orchestrator/runner.py`, call the helper after
      `validate_top_n` and before `rank_candidates`, so the flag is
      persisted on the `Itinerary` row.

## 2. Results payload

- [x] 2.1 In `app/api/runs.py:_build_results_payload`, compute a
      `structures: {"A": "complete"|"incomplete"|"absent",
      "B": "complete"|"incomplete"|"absent"}` map from the candidate
      set.
- [x] 2.2 Extend `ResultsOut` in `app/schemas.py` with a `structures`
      field (mapping `str → str`).

## 3. UI

- [x] 3.1 In `app/templates/run.html`, for each structure in `["A",
...
- [x] 3.2 Verify the existing `Flag.INCOMPLETE` badge style is defined
      in `base.html`; add `.b-INCOMPLETE` if missing.

## 4. Tests

- [x] 4.1 In `tests/test_orchestrator.py`, add a unit test where
...
- [x] 4.2 In `tests/test_orchestrator.py`, add a test where every
      structure has at least one VALIDATED candidate; assert no
      INCOMPLETE flag appears.
- [x] 4.3 In `tests/test_runner_end_to_end.py`, add an assertion that
      the persisted `Itinerary.flags` of an only-LEAD structure
      includes `"INCOMPLETE"`.

## 5. Spec + validate

- [x] 5.1 Update `openspec/specs/itinerary-orchestration/spec.md` with
      the new requirement (see this change's `specs/` delta).
- [x] 5.2 Run `openspec validate flag-incomplete-structures --strict`.
- [x] 5.3 Run `mise run test`.
