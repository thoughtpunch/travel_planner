...
- [ ] 2.1 Confirm FAILED markers already pass through
      ...
      itinerary fares.
- [ ] 2.2 Ensure persisted FAILED `Fare` rows have `price_per_pax=0`,
      `price_party=0`, `verification_status="FAILED"`, and
      `notes=json.dumps({"reason": <reason>, ...})`.

## 3. Results payload

- [ ] 3.1 Add `failed_query_count: int` to `ResultsOut` in
      `app/schemas.py`.
- [ ] 3.2 In `_build_results_payload`, set
      `failed_query_count = sum(1 for f in fare_rows if
      f.verification_status == "FAILED")`.

## 4. UI

- [ ] 4.1 In `app/templates/run.html`, render a "Failed queries
      ({{ failed_query_count }})" disclosure block that lists each
      FAILED `Fare` row (route, date, reason).
- [ ] 4.2 Ensure `.b-FAILED` badge style exists in `base.html`
      (already present).

## 5. Tests

- [ ] 5.1 Add `tests/test_router.py` (new file) or extend
...
- [ ] 5.2 In `tests/test_runner_end_to_end.py`, add a test where the
...
- [ ] 6.1 Update `openspec/specs/fare-search/spec.md` per this
      change's delta in `specs/`.
- [ ] 6.2 Run `openspec validate persist-no-fallback-failed-fares
      --strict`.
- [ ] 6.3 Run `mise run test`.
