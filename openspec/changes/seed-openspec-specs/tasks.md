...
- [ ] 1.1 Replace `openspec/config.yaml` boilerplate with real `context:` block
...
- [ ] 1.2 Add `rules:` for `proposal` and `tasks` artifacts capturing
      project-specific guidance (Phase labelling, failure-mode mapping).

## 2. Seed capability specs

- [ ] 2.1 Create `openspec/specs/fare-search/spec.md` from
      `docs/fare-search.md` — all five requirements, verbatim wording.
- [ ] 2.2 Create `openspec/specs/fare-validation/spec.md` from
      `docs/fare-validation.md` — all three requirements.
- [ ] 2.3 Create `openspec/specs/itinerary-orchestration/spec.md` from
      `docs/itinerary-orchestration.md` — all four requirements.
- [ ] 2.4 Create `openspec/specs/search-config/spec.md` by splitting the
      `search-config` block out of `docs/config-api-ui.md` — both requirements.
- [ ] 2.5 Create `openspec/specs/web-api/spec.md` by splitting the `web-api`
      block out of `docs/config-api-ui.md` — both requirements.
- [ ] 2.6 Create `openspec/specs/web-ui/spec.md` reflecting the *Phase-1
...
- [ ] 4.6 In `README.md`, replace `(24 tests)` with the actual count
      captured from `mise run test`.

## 5. Sanity check

- [ ] 5.1 Run `mise run test` — all tests still pass (no behaviour
      change).
- [ ] 5.2 Confirm `openspec validate seed-openspec-specs --strict`
      passes.
- [ ] 5.3 Confirm `openspec list` shows the change with non-zero task
