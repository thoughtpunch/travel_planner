...
const LEGS = [
  ...
  { leg:2, page:'/turin',       city:'Turin',            dates:'Sep 13–15',    nights:2, people:6,
    lodgingId:'lo-turin',    getHere:{ eur:84,  paid:true, note:'PAID €83.60 — Frecciarossa 9304, PNR NY2TU5, one booking of six' },
  ...
  { leg:3, page:'/cinqueterre', city:'Ligurian coast',   dates:'Sep 15–19',    nights:4, people:6,
    lodgingId:'lo-coast',    getHere:{ eur:68,  paid:true, note:'PAID €67.60 — Intercity 511 direct, PNR N7PZ5N + N7MGC5, split 4+2' },
  ...
  { leg:4, page:'/florence',    city:'Florence',         dates:'Sep 19–26',    nights:7, people:7,
    lodgingId:'lo-florence', getHere:{ eur:113, paid:true, note:'PAID €112.80 — IC 505 + RV via Pisa, PNR N7SKJN + N7S4TN. Party of 7, Grandma rides along' },
  ...
  { leg:5, page:'/rome',        city:'Rome',             dates:'Sep 26–Oct 3', nights:7, people:6,
    lodgingId:'lo-rome',     getHere:{ eur:106, paid:true, note:'PAID €105.80 — Frecciarossa 9415 13:48, PNR N7T7W5 + N7UFZN, FrecciaFAMILY' },
  ...
  { leg:6, page:'/venice',      city:'Venice / Lido',    dates:'Oct 3–10',     nights:7, people:6,
    lodgingId:'lo-venice',   getHere:{ eur:170, note:'⚠️ THE LAST LEG TO BUY — Roma Termini → Venezia S. Lucia, split 4+2' },
...
const out = LEGS.map(L => {
  ...
  const rows = [
    ...
    { key:'getHere',    label:'Getting here',            lo:L.getHere.eur, avg:L.getHere.eur, hi:L.getHere.eur, fixed:true,
      badge: L.getHere.paid ? 'paid' : (L.getHere.eur ? 'to book' : null), note:L.getHere.note },
